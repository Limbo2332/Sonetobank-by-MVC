...
function updateMonthPayment(id1, id2) {
    let monthPayment = $(".month-payment");
    let sum = $(id1).val();
    let term = $(id2).val();
    let p = 0.015;

    if (sum >= 10000 && sum <= 1000000 && term >= 6 && term <= 120) {
        let res = Number(sum * term * p + Number(sum)) / term;
        let expenses = (sum * term * p).toFixed(2);
        let expenses2 = (Number(expenses) + Number(sum)).toFixed(2);
